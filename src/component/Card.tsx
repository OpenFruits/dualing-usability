    ...
    <div className="overflow-hidden bg-white rounded border">
      ...
      <div className="flex justify-start p-2">
        <Link href={`/student/${props.student.uid}`}>
          <a className="mr-2">
            {relation === "no" && <Button className="text-xs">詳細・スカウト画面へ</Button>}
            {relation === "scout" && <Button className="text-xs">詳細画面へ</Button>}
            {relation === "match" && <Button className="text-xs">詳細・チャット画面へ</Button>}
          </a>
        </Link>
        <button
          onClick={bookmark}
        ...
        >
          ...
          <p className="py-2">{isBookmark ? "保存済み" : "保存して後で見る"}</p>
        </button>
      </div>
    </div>
  );
